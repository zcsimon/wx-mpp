...
Page({
  ...
  getNotice: function() {
    var that = this;
    // that.setData({
    //   noticeList: {"dataList":[{"id": "1", "title": "关于鼓励居民接种新冠肺炎疫苗的措施"}, {"id": "2", "title": "真对社区未接种新冠疫苗的居民或将采取强制措施?"}]}
    // });
    api.fetchRequest('/server/mmp/notice/select',{}).then(function (res) {
      if (res.data.code == 0) {
        that.setData({
          noticeList: {"dataList":res.data.data}
        });
      }
    })
  },
